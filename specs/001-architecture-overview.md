...
- All Server-Sent Events (SSE) dispatched on `GET /api/events` are formally defined via JSON Schemas in `api/schemas/` (`event_state.json`, `event_video_state.json`, `event_voice_state.json`, `event_refresh.json`).
