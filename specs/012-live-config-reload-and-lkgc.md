...
3. **SSE Signal Dispatch (`screen.rotate`)**:
   ...
   - For any newly placed widgets on the active screen, the daemon immediately pushes their cached state or renders fresh HTML fragments via `GET /widgets/{widget_id}/render`.
