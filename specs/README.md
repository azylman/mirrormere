...
- **[003-widget-contract.md](003-widget-contract.md)**: Specification for pluggable widgets, data provider schemas, lifecycle hooks, and unified semantic HTML template model.
- **[004-video-cast-ingest.md](004-video-cast-ingest.md)**: Unified Video Stream API (`POST /api/video/trigger`), display video modes, role-based video priority stack (persistent media focus with temporary alert Picture-in-Picture docking), kiosk video sidecars (stock `go2rtc` with hardware transcoding + `sidecars/cast-watcher`), and WebRTC audio architecture.
...
- **[008-tasks-and-lists.md](008-tasks-and-lists.md)**: Checklists, grocery lists and chores: local pure-Go SQLite store (`modernc.org/sqlite`, zero CGO), pluggable source-of-truth adapters (Google Tasks, generic HTTP), ambient read-only display across Touch Kiosk and E-Ink profiles, and SSE updates over the SPEC-006 protocol.
...
- **[012-live-config-reload-and-lkgc.md](012-live-config-reload-and-lkgc.md)**: Live configuration reload and Last-Known-Good Configuration (LKGC) resilience model: directory-level atomic save watching, multi-stage validation pipeline, JIT worker diffing, layout recalculation, and explicit `*_env` secret resolution.
- **[013-implementation-roadmap.md](013-implementation-roadmap.md)**: Phased implementation roadmap and engineering milestones from Day-1 Docker Compose and Go core engine to the web HUD, Ambient E-Ink profile, and Touch Kiosk profile.
